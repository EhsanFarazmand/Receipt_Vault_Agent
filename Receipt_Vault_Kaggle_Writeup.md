...
- **Intake & Extraction Agent** — reads the receipt, sanitizes the untrusted text, extracts `{vendor, date, total, line items, payment last-4, category}`, classifies returnable vs. consumable, and renames/files the source. (Image/PDF OCR runs inside an isolated sandbox as the production step; the working prototype ingests receipt text and e-receipts.)
