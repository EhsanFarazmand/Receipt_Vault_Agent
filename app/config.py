...
MODEL: str = os.environ.get("RECEIPT_VAULT_MODEL", "gemini-flash-latest")

# ---------------------------------------------------------------------------
# Local-first storage layout (privacy pillar: the vault lives on the user's box)
# ---------------------------------------------------------------------------
INBOX_DIR: Path = Path(os.environ.get("RECEIPT_VAULT_INBOX", "./inbox")).expanduser()
VAULT_DIR: Path = Path(os.environ.get("RECEIPT_VAULT_HOME", "./vault")).expanduser()
LEDGER_DB: Path = Path(
    os.environ.get("RECEIPT_VAULT_DB", str(VAULT_DIR / "ledger.sqlite"))
).expanduser()
# Append-only audit trail of what was read, drafted, and sent (Observability pillar).
AUDIT_LOG: Path = Path(os.environ.get("RECEIPT_VAULT_AUDIT", str(VAULT_DIR / "audit.log"))).expanduser()
...
RETURN_WINDOW_THRESHOLD_DAYS: int = int(
    os.environ.get("RECEIPT_VAULT_RETURN_THRESHOLD_DAYS", "7")
)
# Fire a "warranty-expiring" event when an owned item's warranty lapses within N days.
WARRANTY_THRESHOLD_DAYS: int = int(
    os.environ.get("RECEIPT_VAULT_WARRANTY_THRESHOLD_DAYS", "30")
)
# A price drop must clear this dollar delta to be worth a price-protection claim.
PRICE_DROP_MIN_DELTA: float = float(
    os.environ.get("RECEIPT_VAULT_PRICE_DROP_MIN_DELTA", "5.00")
)
